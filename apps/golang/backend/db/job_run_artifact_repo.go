...
func (r *JobRunArtifactRepo) Create(ctx context.Context, a *domain.JobRunArtifact) error {
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO job_run_artifacts (id, tenant_id, job_run_id, job_run_module_id, name, artifact_type, storage_path, size_bytes, content_type, created_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))`,
		a.ID, a.TenantID, a.JobRunID, a.JobRunModuleID, a.Name, a.ArtifactType, a.StoragePath, a.SizeBytes, a.ContentType,
	)
	return err
}
...
func (r *JobRunArtifactRepo) FindByID(ctx context.Context, tenantID, id string) (*domain.JobRunArtifact, error) {
	row := r.db.QueryRowContext(ctx,
		`SELECT id, tenant_id, job_run_id, job_run_module_id, name, artifact_type, storage_path, size_bytes, content_type, created_at
		 FROM job_run_artifacts WHERE tenant_id = ? AND id = ?`, tenantID, id,
	)
	var a domain.JobRunArtifact
	if err := row.Scan(
		&a.ID, &a.TenantID, &a.JobRunID, &a.JobRunModuleID, &a.Name, &a.ArtifactType, &a.StoragePath, &a.SizeBytes, &a.ContentType, &a.CreatedAt,
	); err != nil {
