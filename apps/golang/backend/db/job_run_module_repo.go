...
func (r *JobRunModuleRepo) Create(ctx context.Context, m *domain.JobRunModule) error {
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO job_run_modules (id, tenant_id, job_run_id, job_module_id, status, input_json, output_json, created_at, updated_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))`,
		m.ID, m.TenantID, m.JobRunID, m.JobModuleID, m.Status, m.InputJSON, m.OutputJSON,
	)
	return err
}
...
func (r *JobRunModuleRepo) FindByID(ctx context.Context, tenantID, id string) (*domain.JobRunModule, error) {
	row := r.db.QueryRowContext(ctx,
		`SELECT id, tenant_id, job_run_id, job_module_id, status,
	...
	if err := row.Scan(
		&m.ID, &m.TenantID, &m.JobRunID, &m.JobModuleID, &m.Status,
		&m.InputJSON, &m.OutputJSON, &m.ErrorMessage, &m.StartedAt, &m.FinishedAt,
...
func (r *JobRunModuleRepo) UpdateStatus(ctx context.Context, tenantID, id, status string) error {
	_, err := r.db.ExecContext(ctx,
		`UPDATE job_run_modules SET status = ?, updated_at = datetime('now') WHERE tenant_id = ? AND id = ?`,
		status, tenantID, id,
	)
	return err
}
